...
>
> 小红书封面 · 公众号配图 · 正文插画 · 动作表情 · 多造型角色库 · Codex Skill
...
**Devon Cat Illustrations（德文猫 IP 插画系统）** 是一个为中文内容与个人 IP 设计的 Agent Skill。它会先理解文章里的认知锚点，再把判断、流程、状态或隐喻转换成小红书 3:4 封面、公众号正文配图与头图；也可以生成单角色动作、表情、四季生活、学习工作与运动素材。
...
- 生成固定 1080 × 1440 px 的小红书 3:4 竖版封面。
- 生成固定 1080 × 608 px 的公众号正文配图，以及 900 × 383 px 的首条头图。
- 找出真正值得配图的认知锚点，不按段落机械平均分配。
- 先输出 1–9 张 shot list，再生成，或按用户要求直接出图。
- 为每张图重新设计物理隐喻，让德文猫承担核心动作。
- 通过三视图与五套角色库约束身份、动作、表情、穿搭和配色。
- 保持同一只德文猫身份，同时按内容轮换签名夹克、工作帽衫、针织开衫、居家装、背带裤和运动装。
...
- 小红书封面：1080 × 1440 px、3:4 竖版；允许一个清楚大标题。
- 公众号正文配图：1080 × 608 px、约 16:9；无海报大标题，只留必要短标注。
- 公众号首条头图：900 × 383 px、2.35:1；只有明确说“封面/头图/首图”时启用。
- 通用正文配图：默认 16:9，一篇文章通常 4–8 张。
...
| 发布位置 | 固定工作画布 | 版式重点 | 默认文字 |
|---|---:|---|---|
| 小红书笔记封面 | **1080 × 1440 px**，3:4 | 手机信息流层级、中央方形兼容、安全边距 | 1 个 8–18 字大标题 + 最多 3 个短标签 |
| 公众号正文配图 | **1080 × 608 px**，约 16:9 | 横版认知解释、角色动作、正文阅读节奏 | 0–3 个短标注，不重复文章标题 |
| 公众号首条头图 | **900 × 383 px**，2.35:1 | 中央方形安全区、左右可裁切背景 | 最多 12 字，可不写标题 |

这里的像素是本 Skill 的稳定工作模板，不是对平台全部上传格式的穷举。小红书封面固定使用花海确认的 1080 × 1440 px；公众号只有明确说“封面、头图、首图”时才使用 900 × 383 px，普通“公众号配图”默认指 1080 × 608 px 正文图。

安全区与详细版式见 [平台画布与版式规则](references/platform-formats.md)。比例核对参考了[小红书分享开放平台](https://agora.xiaohongshu.com/doc/ios)和当前的[微信公众号图片尺寸工作指南](https://www.cardcrafter.cn/guides/wechat-cover-sizes)。

---

## 小红书封面案例｜3 组

三组封面分别验证判断型、共鸣型和教程型版式。角色身份保持一致，但服装、姿态和内容动作主动轮换，不再全部回落到绿色夹克。

<table>
  <tr>
    <td width="33%" valign="top">
      <img src="examples/platform-formats/xhs/01-ai-judgment-cover.png" alt="AI 不是答案，判断才是">
      <br><strong>判断型｜AI 不是答案</strong><br>
      签名绿色夹克；用筛选台把“AI 答案”和人的判断分开。
    </td>
    <td width="33%" valign="top">
      <img src="examples/platform-formats/xhs/02-energy-low-cover.png" alt="你不是懒，只是能量见底了">
      <br><strong>共鸣型｜能量见底</strong><br>
      居家恢复造型；用低电量灯和充电动作表达“先休息，再出发”。
    </td>
    <td width="33%" valign="top">
      <img src="examples/platform-formats/xhs/03-ai-workflow-cover.png" alt="3 步搭好 AI 工作流">
      <br><strong>教程型｜3 步工作流</strong><br>
      蓝色工作帽衫；亲手连接输入、处理、输出三个模块。
    </td>
  </tr>
</table>

共同规格：1080 × 1440 px、3:4 竖版、四边约 8% 安全边距、标题逐字校对、每张独立输出。

---

## 公众号正文配图案例｜3 组

公众号正文图不再做成缩小版海报。每张用一个横版动作解释正文关系，同时继续轮换角色库中的完整造型。

<table>
  <tr>
    <td width="33%" valign="top">
      <img src="examples/platform-formats/wechat/01-learning-loop.png" alt="学习闭环">
      <br><strong>认知闭环｜输入·输出·复盘</strong><br>
      绿色针织开衫；角色一边写输出，一边转动学习闭环。
    </td>
    <td width="33%" valign="top">
      <img src="examples/platform-formats/wechat/02-anxiety-unpack.png" alt="焦虑拆解">
      <br><strong>状态拆解｜把焦虑变小</strong><br>
      条纹衫与蓝色背带裤；把线团整理成一步行动。
    </td>
    <td width="33%" valign="top">
      <img src="examples/platform-formats/wechat/03-action-start.png" alt="行动之后才有动力">
      <br><strong>行动变化｜先动一下</strong><br>
      蓝白运动装；推动第一个方块，让后续齿轮开始运转。
    </td>
  </tr>
</table>

共同规格：1080 × 608 px、横版、0–3 个短标注、没有海报式大标题。完整案例说明见 [平台案例库](examples/platform-cases.md)。
...
仓库内置 16 个认知隐喻案例和 6 个平台成品案例。可复用的 [德文猫认知案例](examples/case-library.md) 覆盖内容创作、产品验证、学习成长、信任建立、情绪状态与行动系统；[平台案例库](examples/platform-cases.md) 则展示小红书与公众号的具体尺寸、版式和造型轮换。
...
固定为 3:4 竖屏、1080 × 1440 px；三张分别使用判断型、共鸣型、教程型版式。
保持同一德文猫身份，但三张使用三套明显不同的完整造型，不要都穿绿色夹克。
...
```text
Use $huahai-cat-illustrations 为这篇公众号文章生成 3 张正文配图。
固定为 1080 × 608 px 横版；每张只解释一个认知关系，不放海报式大标题。
短标注最多 3 个，三张至少轮换两套完整造型。
...
固定为 900 × 383 px、2.35:1；把德文猫脸部和标题关键词放在中央方形安全区。
...
文章 / 观点 / 平台封面 / 角色素材需求
    ↓
识别小红书、公众号正文、公众号头图或通用模式
    ↓
固定三视图身份主参考
    ↓
按任务选择一张场景副参考，并为批量图片分配不同完整造型
    ↓
确定目标像素、标题密度、安全区和核心动作
    ↓
附带主参考和副参考逐张生成
    ↓
检查尺寸、身份、造型轮换、文字、肢体、构图和宫格污染
    ↓
保存 PNG，并按需插入正文
```
...
│   └── platform-formats/
│       ├── xhs/
│       │   ├── 01-ai-judgment-cover.png
│       │   ├── 02-energy-low-cover.png
│       │   └── 03-ai-workflow-cover.png
│       └── wechat/
│           ├── 01-learning-loop.png
│           ├── 02-anxiety-unpack.png
│           └── 03-action-start.png
└── references/
    ├── character-ip.md
...
- 绿色夹克是签名造型，不是全局制服；批量图片必须先分配造型，再逐张生成。
- 小红书封面与公众号正文图是两套版式逻辑，不能只靠裁切互换。
