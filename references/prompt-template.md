...
每张图单独调用当前环境可用的图像生成或编辑工具，并把 `../assets/devon-cat-character-sheet.png` 作为 reference image 传入。不要只在文字中提到参考图而不实际附带它。
...
Composition:
{德文猫的位置、动作、主要物件、信息如何变化}

Chinese handwritten labels:
{3–6 个短标注}

Visual DNA:
Pure or near-pure white background. Clean hand-drawn line art with slight natural wobble. Lots of empty space. Keep the Devon Cat in its restrained canonical colors. Use forest green for primary structure, warm orange for the key action or path, deep blue for secondary system notes, and warning red only when necessary. The whole subject occupies about 40%–65% of the canvas.
...
```text
Redesign this article illustration around the same core idea, using the attached Devon Cat sheet as the only character identity reference. Replace the old actor with Devon Cat and adapt the action so the cat genuinely drives the concept. Do not trace, recolor, or paste the new character over the old one. If the old composition depends on a Xiaohei-specific metaphor, invent a new low-tech metaphor while preserving the intended meaning. Keep a 16:9 white-background hand-drawn article-illustration style.
```
